...
reynoldsSSTRoot="../data/reynolds_sst/reynolds_avhrr_only_monthly_resampled_1.0x1.0/"
...
#
#
#Global SOCATv2019 ascii ############### Test case with north pacific
#datasetName="SOCATv2019" #Input data should be in a sub-directory which matched this name.
# python $fluxEngineRoot"fluxengine_src/tools/reanalyse_socat_driver.py" -socat_dir $inputDataPath$datasetName"/" -socat_files $datasetName"_NorthPacific.tsv" -sst_dir $reynoldsSSTRoot -sst_tail $reynoldsSSTTail -output_dir $outputDirectory$datasetName"/output_SOCATv2019_ascii_mini" -socatversion 6 -usereynolds -startyr 1957 -endyr 2019 -keepduplicates -keeptempfiles -asciioutput
#Merge reanalysed data into original text file.
#python combine_ascii.py --socatAsciiPath $inputDataPath$datasetName"/"$datasetName"_NorthPacific.tsv" --reanalysisDataDirectory $outputDirectory$datasetName"/output_SOCATv2019_ascii_mini/output/reanalysed_data/" --outputPath "output/merged/"$datasetName"_mini.tsv" --startYear "1981" --stopYear "2019" --suffix="v6"
#python add_header_rename_columns.py --reanalysedInputFile "output/merged/"$datasetName"_mini.tsv" --reanalysedOutputFile "output/merged/"$datasetName"mini_with_header.tsv" --socatAsciiFile "input_data/"$datasetName"/"$datasetName"_NorthPacific.tsv"
#
###python combine_ascii.py --socatAsciiPath input_data/SOCATv2019/SOCATv2019_NorthPacific.tsv --reanalysisDataDirectory output/SOCATv2019/output_SOCATv2019_ascii_mini/output/reanalysed_data/ --outputPath output/merged/SOCATv2019_mini.tsv --startYear 1981 --stopYear 2019 --suffix=v6
#
#
#
...
# datasetName="SOCATv2019" #Input data should be in a sub-directory which matched this name.
# python $fluxEngineRoot"fluxengine_src/tools/reanalyse_socat_driver.py" -socat_dir $inputDataPath$datasetName"/" -socat_files $datasetName".tsv" -sst_dir $reynoldsSSTRoot -sst_tail $reynoldsSSTTail -output_dir $outputDirectory$datasetName"/output_SOCATv2019_netCDF" -socatversion 6 -usereynolds -startyr 1957 -endyr 2019 -keepduplicates -keeptempfiles  --suffix="v6"
